...
class Encoder(torch.nn.Module):
    def __init__(self, name, hiddens, heads, activation, feat_drop, attn_drop, negative_slope, bias):
        ...
        for l in range(0, self.num_layers):
            ...
            elif self.name == "pairre":
                self.func = PairRE(p=p, feat_drop=self.feat_drop, dim=params["dim"], params=self.margin)
...
class PairRE(torch.nn.Module):
    ...
    def forward(self, e1, r, e2):
        ...
        score = e1 * r - e2 * r
        pred = torch.norm(score, p=1, dim=-1)
        return pred
